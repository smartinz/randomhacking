class ExpenseEditController
  def initialize(data_layer, expense)
    glade = GladeXML.new(File.join(File.dirname(__FILE__), 'expense_edit_dialog.glade')) { |handler| method(handler) }
    @expense_edit_dialog = glade['expense_edit_dialog']
    @expense_date_entry = glade['expense_date_entry']
    @amount_entry = glade['amount_entry']
    @expense_type_combobox = glade['expense_type_combobox']
    @payment_form_combobox = glade['payment_form_combobox']
    @description_entry = glade['description_entry']
    @accounting_month_entry = glade['accounting_month_entry']
    @number_accounting_month_entry = glade['number_accounting_month_entry']
  ...
  def open(parent_window)
  
    while true
      @expense_edit_dialog.run do |response|
...
private
  ...
  def save_expense
    begin
      ...
      show_error_dialog(@expense_edit_dialog.transient_for, $!)
      return false
    end
    return true
  end
  ...
  def setup_expense
    @expense_edit_dialog.title = "Edit [#{@expense.to_s}]"
