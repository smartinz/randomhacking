using NorthwindWeb.Business;

namespace NorthwindWeb.Data
{
    public class ShipperRepository
    {
        private readonly Context _context;

        public ShipperRepository(Context context)
        {
            _context = context;
        }

        public void Create(Shipper v)
        {
            Shipper r = _context.NorthwindWebService.CreateShipper(v);

            v.ShipperID = r.ShipperID;
            v.CompanyName = r.CompanyName;
            v.Phone = r.Phone;
        }

        public Shipper Read(int ShipperID)
        {
            return _context.NorthwindWebService.ReadShipper(ShipperID);
        }

        public void Update(Shipper v)
        {
            Shipper r = _context.NorthwindWebService.UpdateShipper(v);

            v.ShipperID = r.ShipperID;
            v.CompanyName = r.CompanyName;
            v.Phone = r.Phone;
        }

        public void Delete(Shipper v)
        {
            _context.NorthwindWebService.DeleteShipper(v);
        }

        public Shipper[] Search(string companyName, string phone)
        {
            return _context.NorthwindWebService.SearchShipper(companyName, phone);
        }
    }
}